...
if($fetch_from_db)
{
	$players_db = $db->query('SELECT `name`, `level`, `experience` FROM `players` WHERE `id` NOT IN (' . implode(', ', $config['highscores_ids_hidden']) . ') AND `group_id` < ' . $config['highscores_groups_hidden'] . ' AND `id` > 6 ORDER BY `experience` DESC LIMIT 5;');
